...
import { Briefcase, Plus, Trash2, Check, ChevronDownIcon, Pencil } from 'lucide-react';
...
function DateInput({ value, onChange, placeholder }: { value?: Date, onChange: (date?: Date) => void, placeholder?: string }) {
...
export function ExperienceSection({ experience, setExperience }: ExperienceSectionProps) {
    ...
    const [currentExperience, setCurrentExperience] = useState<Experience>({ title: '', company: '' });
                            ...
                            <CardContent className="p-4 flex justify-between items-start">
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                                        {exp.start_date && <span>{format(new Date(exp.start_date), 'MMM yyyy')}</span>}
                                        {(exp.start_date || exp.end_date) && <span>-</span>}
                                        {exp.end_date ? <span>{format(new Date(exp.end_date), 'MMM yyyy')}</span> : (exp.start_date && <span>Present</span>)}
                                    </div>
                        ...
                        <div className="p-4 border border-border/40 rounded-2xl space-y-4 bg-muted/5 animate-in fade-in slide-in-from-top-2">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <DateInput
                                    ...
                                    onChange={(date) => setCurrentExperience({ ...currentExperience, start_date: date })}
                                    placeholder="Start Date"
                                />
                                <DateInput
                                    value={currentExperience.end_date}
                                    onChange={(date) => setCurrentExperience({ ...currentExperience, end_date: date })}
                            ...
                            <div className="flex justify-end gap-2">
                                ...
                                <Button size="icon" onClick={saveExperience} disabled={!currentExperience.title || !currentExperience.company}>
